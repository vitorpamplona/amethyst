...
val generateAndroidResourceTable by tasks.registering(GenerateAndroidResourceTable::class) {
    ...
    packageName.set("com.vitorpamplona.amethyst.shared")
    tableDir.set("amethyst-res")
    packageByte.set(0x7f)
}

// :commons ships its own Crowdin-translated `res/values-*` tree and its own
// `com.vitorpamplona.amethyst.commons.R`, which the app references as
// `CommonsR`. It gets the same treatment, from the same task, with its own id
// space (package byte 0x7e) so both tables can be merged at runtime. The res
// tree is read by path rather than through project(":commons") because it is a
// plain source directory with nothing generating it — there is no task to
// depend on.
val commonsResSourceDir: Directory = rootProject.layout.projectDirectory.dir("commons/src/androidMain/res")

val generateCommonsResourceTable by tasks.registering(GenerateAndroidResourceTable::class) {
    group = "build"
    description = "Generate the JVM R class and locale string tables from :commons's Android res/ tree."
    resDir.set(commonsResSourceDir)
    kotlinOutputDir.set(commonsGeneratedResRoot.map { it.dir("kotlin") })
    resourceOutputDir.set(commonsGeneratedResRoot.map { it.dir("resources") })
...
abstract class GenerateAndroidResourceTable : DefaultTask() {
    ...
    /** Kotlin package the generated `R` lands in. */
    @get:Input
    abstract val packageName: Property<String>

    /** Classpath directory the runtime tables are read from. */
    @get:Input
    abstract val tableDir: Property<String>

    /**
     * aapt2's id layout: 0x<package><type><entry>. On Android the package byte
     * is per-APK and cosmetic; here it is what keeps two `R` classes generated
     * from two res trees out of each other's id space, so one merged runtime
     * table can serve both.
     */
    @get:Input
    abstract val packageByte: Property<Int>

    private fun resourceId(typeId: Int, index: Int) =
        (packageByte.get() shl 24) or (typeId shl 16) or index
    ...
    private fun writeRClass(
        ...
        sb.append("// Generated by :amethystShared. DO NOT EDIT.\n")
    ...
    private fun writeTables(
        ...
        val root = File(resourceOutputDir.get().asFile, tableDir.get())
    ...
    private fun copyDrawables(
        ...
        val outRoot = File(resourceOutputDir.get().asFile, tableDir.get() + "/drawable")
        outRoot.mkdirs()
        ...
        File(resourceOutputDir.get().asFile, tableDir.get() + "/drawables.tsv").writeText(index.toString())
    }
...
// ============================================================================
// BuildConfig for the JVM target
// ============================================================================
// AGP generates `com.vitorpamplona.amethyst.BuildConfig` for the Android app.
// The JVM has no AGP, so generate the same class with the same field names,
// sourced from the same version catalog so VERSION_NAME cannot drift. It is
// emitted into jvmMain only — the Android target keeps AGP's.
val generateJvmBuildConfig by tasks.registering {
    val appVersion = libs.versions.app.get()
    val releaseNotesId = "00d306e01792e48b93638b73b57a7eb8b89622a338b1b4f529622150d46cd710"
    val outDir = layout.buildDirectory.dir("generated/buildConfig/kotlin")
    outputs.dir(outDir)
    inputs.property("appVersion", appVersion)

    doLast {
        val file = outDir.get().file("com/vitorpamplona/amethyst/BuildConfig.kt").asFile
        file.parentFile.mkdirs()
        file.writeText(
            """
            |// Generated by :amethystShared:generateJvmBuildConfig. DO NOT EDIT.
            |package com.vitorpamplona.amethyst
            |
            |/** JVM counterpart to the AGP-generated BuildConfig. */
            |public object BuildConfig {
            |    public const val VERSION_NAME: String = "$appVersion"
            |    public const val BUILD_TYPE: String = "release"
            |    public const val FLAVOR: String = "desktop"
            |    public const val DEBUG: Boolean = false
            |    public const val RELEASE_NOTES_ID: String = "$releaseNotesId"
            |
            |    /** Google Cast is Android-only; the desktop build hides the feature. */
            |    public const val IS_CASTING_AVAILABLE: Boolean = false
            |}
            |
            """.trimMargin(),
        )
