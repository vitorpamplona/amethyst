...
- **No capability gate yet (Phase 3).** "NWC wallet present" is the proxy for
  "wallet supports `pay`". If the wallet lacks `pay`, a BOLT12-first recipient's zap
  fails with the wallet's error rather than falling back to lightning. Reading
  `get_info.methods` / the 13194 info event to gate (and to auto-fall-back) is the
  next step.
