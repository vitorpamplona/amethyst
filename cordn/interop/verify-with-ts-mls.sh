...
#
# `:cordn`'s own tests prove we can read what ts-mls writes. That is half of
...
#
# Usage:
#   cordn/interop/verify-with-ts-mls.sh
#
# Environment:
...
OUT="$REPO/cordn/build/interop"
FIXTURES="$REPO/cordn/src/jvmTest/resources/tsmls"
...
echo "==> producing artifacts from :cordn"
"$REPO/gradlew" -p "$REPO" :cordn:jvmTest --tests '*KotlinArtifactProducerTest*' --rerun-tasks -q
