...
| [2026-09-25-nostr-sql-profile.md](2026-09-25-nostr-sql-profile.md) | Read-only SQL over websockets: SQLite SELECT as the spec, `events`/`tags` virtual tables, parse + re-emit with per-session table substitution. Parser/compiler differential-fuzzed against SQLite; `SQL`/`FETCH`/`SQL-CLOSE` always on in `NostrServer` over file-backed SQLite (read-only connections, REQ policy gate, first page = default limit). Driver has no interrupt; indexed tags and client helpers open. |
